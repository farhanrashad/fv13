        ...
        for order in self:
            ...
            for prod in production_lines:
                if prod.product_id.activate_production == True:
                    existing_bill = self.env['account.move'].search(['&', ('partner_id', '=', prod.product_id.partner_id.id), ('state', '=', 'draft')])
                    existing_bill_lines = self.env['account.move.line'].search([('invoice_id', '=', existing_bill.id)])
                    if existing_bill:
