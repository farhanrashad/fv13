...
order by 1
        ...
        query3 = """
        select a.sale_order, a.product_category, max(a.uom) as uom, a.location, sum(a.quantity) as quantity, sum(a.product_weight) as product_weight from (
select l.product_id, pt.name as product_name, um.name as uom, pt.categ_id, pc.name as product_category, m.partner_id, so.name as sale_order, l.sale_id, sl.name as location, lot.name as lot, l.qty_done as quantity, coalesce(l.total_weight,0) as product_weight 
from stock_move m
join stock_move_line l on l.move_id = m.id
...
where m.state in ('done' ) and l.date <=  %(date)s and (l.location_dest_id = %(location_id)s or l.location_dest_id in (select z.id from stock_location z where z.location_id = %(location_id)s ) )
union all
select l.product_id, pt.name as product_name, um.name as uom, pt.categ_id, pc.name as product_category, m.partner_id, so.name as sale_order, l.sale_id, sl.name as location, lot.name as lot, l.qty_done*-1 as quantity, coalesce(l.total_weight,0)*-1 as product_weight 
from stock_move m
join stock_move_line l on l.move_id = m.id
join sale_order so on l.sale_id = so.id
join product_product pp on l.product_id = pp.id
...
group by a.sale_order, a.product_category, a.location
order by 1
        """ 
        query4 = """
        select a.partner, a.product_category, max(a.uom) as uom, a.location, sum(a.quantity) as quantity, sum(a.product_weight) as product_weight from (
select l.product_id, pt.name as product_name, um.name as uom, pt.categ_id, pc.name as product_category, rp.name as partner, m.partner_id, l.sale_id, sl.name as location, lot.name as lot, l.qty_done as quantity, coalesce(l.total_weight,0) as product_weight 
from stock_move m
...
where m.state in ('done' ) and l.date <=  %(date)s and (l.location_dest_id = %(location_id)s or l.location_dest_id in (select z.id from stock_location z where z.location_id = %(location_id)s ) )
union all
select l.product_id, pt.name as product_name, um.name as uom, pt.categ_id, pc.name as product_category, rp.name as partner, m.partner_id, l.sale_id, sl.name as location, lot.name as lot, l.qty_done*-1 as quantity, coalesce(l.total_weight,0)*-1 as product_weight 
from stock_move m
join res_partner rp on m.partner_id = rp.id
join stock_move_line l on l.move_id = m.id
join product_product pp on l.product_id = pp.id
...
group by a.partner, a.product_category, a.location
order by 1
        """ 
        ...
        self.env.cr.execute(query3, params=params) 
        dat3 = cr.dictfetchall()
        
        self.env.cr.execute(query4, params=params) 
        dat4 = cr.dictfetchall()
