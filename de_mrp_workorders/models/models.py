...
class MrpProduction(models.Model):
    ...
    def onchange_routing(self):
        ...
        if self.routing_f_id.id == 20:  
            self.update ({
                  'routing_s_id': 19,
                  'routing_t_id': 18,
                  'routing_fo_id': 17,                  
                	}) 
        if self.routing_f_id.id == 17:  
            self.update ({
                  'routing_s_id': 19,
                  'routing_t_id': 18,
                  'routing_fo_id': 20,                  
                	})     
        if self.routing_f_id.id == 21:  
            self.update ({
                  'routing_s_id': 22,
                  'routing_t_id': 23,
                  'routing_fo_id': 24,                  
    ...
    def button_plans(self):
        ...
        if self.product_qty < total_quantity:
            raise exceptions.ValidationError('Routing Quantity must be equal to Quantity to Consume.')
