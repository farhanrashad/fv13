...
class ProductImportUpdate(models.TransientModel):
    ...
    one_time_import = fields.Boolean(string='First Time Import',help="If you have large no. of products on your woocommerce  site then enable this It will import product page by page",default = False)
...
class CategoryExportUpdate(models.TransientModel):
    ...
    def process(self):
        if 'active_id' in self._context:
            ...
            if channel:
                if self.import_update == 'export':
                    message = channel.export_woocommerce_categories(0)
                else:
